...
- **保存 / 切换账号**：快照 credentials + config 双文件，原子替换；切换前自动保全当前登录，绝不丢号；设备身份跟账号走，远程控制中继密钥跨切换保活
- **添加账号**：工具内 OAuth 登录新号（BigModel / z.ai 双入口），全程不动当前登录
- **额度展示**：账号行内联显示套餐额度与重置时间，多套餐分组、错峰轮询
- **活动领取**：可领套餐一键领取（GUI 验证码）；「刷新领取资格」按钮逐号手动激活活动资格（限流冷却，进度 1/N）
...
全部拒绝即可，不影响任何功能，验证码照常通过。
应用内嵌的登录/验证码网页由系统 WebView 渲染，网页发起的请求会被透传为应用的系统权限询问；应用本体与其内嵌网页均未使用这三项能力。
